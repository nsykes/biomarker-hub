...
export function ResultsPanel({
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header bar */}
      <div className="flex items-center gap-3 px-3 py-2.5 border-b border-[var(--color-border-light)] bg-[var(--color-surface)] flex-shrink-0 flex-wrap" style={{ boxShadow: 'var(--shadow-sm)' }}>
        <button
          onClick={() => {
      ...
      <div className="flex-1 overflow-auto">
        ...
        {extraction && extraction.biomarkers.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full px-6 py-12">
            <div className="flex flex-col items-center gap-3 max-w-sm text-center px-6 py-8 rounded-2xl" style={{ background: 'var(--color-warning-bg)' }}>
              <svg className="w-10 h-10 text-[var(--color-warning-text)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <p className="text-[var(--color-warning-text)] font-semibold">No biomarkers found</p>
              <p className="text-sm text-[var(--color-warning-text)] opacity-80">
                This PDF doesn&apos;t appear to contain lab results, or the format wasn&apos;t recognized. Try a different file, or re-attempt with another model.
              </p>
            </div>
          </div>
        )}
        ...
        {extraction && extraction.biomarkers.length > 0 && (
          <>
            <table className="w-full text-left">
              <thead className="sticky top-0 bg-[var(--color-surface)] border-b border-[var(--color-border-light)] z-10">
                ...
                {calculatedBiomarkers.length > 0 && (
                  <>
                    <tr
                      ...
                      className="bg-[var(--color-calculated-bg)] cursor-pointer hover:bg-[var(--color-calculated-bg-hover)] transition-colors"
                    >
                      <td
                        colSpan={7}
                        className="px-2 py-1.5 font-semibold text-sm text-[var(--color-calculated)]"
                      >
                        <span className="inline-flex items-center gap-1.5">
                          <svg
                            className={`w-3 h-3 text-[var(--color-calculated)] transition-transform duration-200 ${isCollapsed("calc") ? '' : 'rotate-90'}`}
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                          >
                          ...
                          <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-[var(--color-calculated-badge-bg)] text-[var(--color-calculated)] uppercase tracking-wider">
                            CALC
                          </span>
                        </span>
                      </td>
